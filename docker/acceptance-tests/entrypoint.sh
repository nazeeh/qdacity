...
sudo /usr/local/gcloud/google-cloud-sdk/bin/java_dev_appserver.sh --disable_update_check --port=8888 /app/target/qdacity-war/ 2>&1 | sudo tee /app/target/devserver.log > /dev/null &
...
cd realtime-service
npm run start 2>&1 | sudo tee /app/realtime-service/rtcs.log > /dev/null &
cd ..
