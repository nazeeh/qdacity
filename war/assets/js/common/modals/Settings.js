...
export default class Settings extends VexModal {
	...
	showModal(umlEditorEnabled, projectId) {
		...
		var promise = new Promise(function(resolve, reject) {
			...
			formElements +=
				'<label><input id="settingsUmlEditorEnabled" type="checkbox" value=""' +
				checked +
				'>' +
				formatMessage({
					id: 'settings.enable_uml',
					defaultMessage: 'Enable UML Editor'
				}) +
				'</label>';
			formElements += '</div>';
					...
					$.extend({}, vex.dialog.buttons.YES, {
						...
						click: function($vexContent) {
							...
							for (var i in changeWeightNames) {
								var changeWeightInputValue = $('#cell' + i + '-1-input').prop(
									'value'
								);
								var saturationMaxInputValue = $('#cell' + i + '-2-input').prop(
									'value'
								);
								if (typeof changeWeightInputValue === 'undefined') {
									//if true, then we applied an average, so lets look up the average
									var categoryIdx = saturationWeights.getArtificialCategoryIndex(
										saturationWeights.getCategoryForIndex(i)
									);
									var newAverageWeight = $(
										'#input-category-' + categoryIdx + '-1'
									).prop('value');
									var oldAverageWeight = $(
										'#input-category-' + categoryIdx + '-1-old'
									).prop('value');
									var newAverageMaxInput = $(
										'#input-category-' + categoryIdx + '-2'
									).prop('value');
									var oldAverageMaxInput = $(
										'#input-category-' + categoryIdx + '-2-old'
									).prop('value');
									var oldChangeWeightInputValue = $(
										'#cell' + i + '-1-input-old'
									).prop('value');
									var oldSaturationMaxInputValue = $(
										'#cell' + i + '-2-input-old'
									).prop('value');
									...
									if (oldAverageWeight > 0) {
										factorWeight =
											1 +
											(newAverageWeight - oldAverageWeight) / oldAverageWeight;
									}
									var factorMax = 1;
									if (oldAverageMaxInput > 0) {
										factorMax =
											1 +
											(newAverageMaxInput - oldAverageMaxInput) /
												oldAverageMaxInput;
									}

									changeWeightInputValue =
										oldChangeWeightInputValue * factorWeight;
									saturationMaxInputValue =
										oldSaturationMaxInputValue * factorMax;
								}

								saturationParameters[changeWeightNames[i]] =
									changeWeightInputValue / 100;
								saturationParameters[saturationMaximumNames[i]] =
									saturationMaxInputValue / 100;
							}
							saturationParameters['lastSatResults'] = $(
								'#saturation-interval'
							).prop('value');
							saturationParameters['projectId'] = projectId;
							new SaturationEndpoint().setSaturationParameters(
			...
			ReactDOM.render(
				...
				document.getElementById('saturationSettings')
			);
		});
