...
export default class ProjectEndpoint {
	...
	static listValidationCoders(prjId){
		var apiMethod = gapi.client.qdacity.user.listValidationCoders({'validationProject': prjId});
	...
	static listUser(prjId){
		var apiMethod = gapi.client.qdacity.user.listUser({'projectID': prjId});
		return Promisizer.makePromise(apiMethod);
	}
	
	static findUsers(searchTerm){
		var apiMethod = gapi.client.qdacity.user.findUsers({'searchTerm': searchTerm});
		return Promisizer.makePromise(apiMethod);
	}
