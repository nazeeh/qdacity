...
export default class ImageChooser extends Component {
	...
	onImageSelected(imgPath) {
		...
		if (
			!(
				imgPath.name.endsWith('.png') ||
				imgPath.name.endsWith('.PNG') ||
				imgPath.name.endsWith('.jpg') ||
				imgPath.name.endsWith('.JPG') ||
				imgPath.name.endsWith('.jpeg') ||
				imgPath.name.endsWith('.JPEG')
			)
		) {
			// not an image
	...
	async onUpload() {
		const scaledImgBase64 = await ImageUtil.scaleToBase64(
				...
				<ImageChooseWrapper>
					...
					<StyledUploadButtonWrapper>
						<StyledUploadButton
							...
							className="vex-dialog-button vex-dialog-button-secondary"
						>
					...
					<StyledFileInputWrapper>
						...
						<input
							id="image-input"
							type="file"
							onChange={e => this.onImageSelected(e.target.files[0])}
						/>
					</StyledFileInputWrapper>
				</ImageChooseWrapper>
			</div>
