...
export default class ProfileSettings extends Component {
	...
	onDeleteUser() {
		...
		vex.dialog.open({
			...
			input: [
				`<label for="deleteInput">${deleteLabel +
					' DELETE'}</label><input name="deleteInput" type="text" required />`
			].join('\n'),
			buttons: [
				...
				if (data.deleteInput !== 'DELETE') {
					vex.dialog.open({
						message: formatMessage({
							...
							defaultMessage:
								'The input was not correct. Aborting the deletion process!'
						}),
						buttons: [
							$.extend({}, vex.dialog.buttons.NO, {
								text: formatMessage({
	...
	async deleteUser() {
		...
		let resultMessage = '';
		try {
			const resp = await UserEndpoint.removeUser(
				this.props.auth.userProfile.qdacityId
			);
			...
			resultMessage = formatMessage({
				...
				defaultMessage:
					'Oops, something went wrong while deleting your account...'
			});
		}
	...
	onChangeNameAndEmail() {
		...
		vex.dialog.open({
			...
			input: [
				`<label for="emailInput">${emailLabel}</label><input name="emailInput" value=${
					_this.props.auth.userProfile.email
				} type="text" required />`,
				`<label for="firstnameInput">${firstNameLabel}</label><input name="firstnameInput" value=${
					_this.props.auth.userProfile.firstname
				} type="text" required />`,
				`<label for="lastnameInput">${lastNameLabel}</label><input name="lastnameInput" value=${
					_this.props.auth.userProfile.lastname
				} type="text" required />`
			].join('\n'),
			buttons: [
				$.extend({}, vex.dialog.buttons.YES, {
		...
		ReactDOM.render(
			<IntlProvider>
				<ThemeProvider theme={Theme}>
					<ImageChooser
						onSave={_this.changeProfileImg}
						initialImg={_this.props.auth.userProfile.picSrc}
					/>
				</ThemeProvider>
			</IntlProvider>,
			...
			<div>
				...
				<StyledPanel>
					...
					<StyledDeleteAccountButton
						id="profile-settings-delete-button"
