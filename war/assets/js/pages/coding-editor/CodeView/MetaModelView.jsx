...
export default class MetaModelView extends React.Component {
  ...
  initProcessEntities(entities, metaModelRelations) {
    ...
    for (let i = 0; i < entities.length; i++) {
      let element = new MetaModelElement(
        entities[i].id,
        entities[i].name,
        entities[i].type,
        entities[i].group
    ...
    for (let i = 0; i < relations.length; i++) {
      ...
      switch (relation.type) {
        case "Association":
          break;
        case "Generalization":
          if (entitiesById.hasOwnProperty(relation.src))
            entitiesById[relation.src].addGeneralization(relation.dst);
          if (entitiesById.hasOwnProperty(relation.dst))
  ...
  render() {
    ...
    if (_this.props.elements != null) {
      return (
        <StyledMetaModelView>
          {Object.keys(_this.props.elements).map((key, index) =>
            _this.renderGroup(_this.props.elements[key], key)
          )}
        </StyledMetaModelView>
      );
    } else {
  ...
  renderButton(mmElement, name, selected) {
    if (selected) {
      return (
        <BtnPrimary
          onClick={this.props.updateActiveElement.bind(null, mmElement)}
          active={true}
        >
          {name}
        </BtnPrimary>
  ...
  renderGroup(elements, group) {
    ...
    let firstLevel = elements.map(function(mmElement) {
      if (
        mmElement.generalizations.length == 0 &&
        (typeof _this.props.filter == "undefined" ||
          _this.props.filter == mmElement.type)
      ) {
        drawFirstLevel = true;
