...
export default class DocumentParser {
    ...
    parseDocument(document) {
        ...
        // Init document
        this.codingResult.addDocument(new CodingDocument(this.document, this.getTotalCodingsCount(this.code.codeID)));
        ...
        for (let i = 0; i < this.allCodeIds.length; i++) {
            ...
            if (codeId != this.code.codeID) {
                codingDocument.createEntry(codeId, new CodingOverlapCollection(codeId, this.getTotalCodingsCount(codeId)));
            }
        }
    ...
    evaluateCodingOverlap(mainCoding, coding) {
        ...
        const exists = codingDocument.hasCodingOverlap(codeId, codingIdMain, codingIdOther);

        if (!exists) {
            const codingOverlap = new CodingOverlap(codingIdMain, codingIdOther);

            const textContent = this.buildTextContent(codingIdMain, codingIdOther, codingOverlap);
        ...
        let relevantParagraphs = this.allParagraphs.filter((index, paragraph) => {
            ...
            const evaluateElement = (element) => {
                if (isValidCoding(element)) {
                    if (element.attribs.id == codingIdMain || element.attribs.id == codingIdOther) {
                        return true;
                    }
                }
                if (element.children) {
