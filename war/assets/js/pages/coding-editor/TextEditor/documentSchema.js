		...
		serialize(obj, children) {
			if (obj.object === 'block') {
				switch (obj.type) {
					case 'paragraph':
						return (
							<p falsenegcount={obj.data.get('falsenegcount')}>{children}</p>
						);
					default:
						return;
				}
	...
	// Font size tag rule
	{
		...
		serialize(obj, children) {
			if (obj.object === 'mark' && obj.type === 'fontsize') {
				return (
					<span className="size" style={{ fontSize: obj.data.toJS().size }}>
						{children}
					</span>
				);
			}
	...
	// Font face tag rule
	{
		...
		serialize(obj, children) {
			if (obj.object === 'mark' && obj.type === 'fontface') {
				return (
					<span className="font" style={{ fontFamily: obj.data.toJS().font }}>
						{children}
					</span>
				);
			}
