...
import GroupTitleRow from './GroupTitleRow.jsx';
...
export default class GroupDashboard extends Component {
	...
	init(userGroupId) {
		...
		UserGroupEndpoint.getUserGroupById(userGroupId).then(function(resp) {
			_this.setState({
				userGroup: resp,
				isOwner: (resp.owners || []).includes(
					_this.props.auth.userProfile.qdacityId
				),
				isParticipant: (resp.isParticipant || []).includes(
					_this.props.auth.userProfile.qdacityId
				)
			});
		});
