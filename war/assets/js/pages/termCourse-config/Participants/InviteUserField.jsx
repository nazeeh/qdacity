...
export default class InviteUserField extends React.Component {
	...
	inviteUser() {
		...
		CourseEndPoint.inviteUserTermCourse(this.props.termCourse.id, this.state.userEmail).then(function () {
			alertify.success(
				formatMessage({id: 'inviteuserfield.invite_success', defaultMessage:  '{email} has been invited'}, {email: _this.state.userEmail})
			);
		}).catch(function () {
			alertify.error(
				formatMessage({id: 'inviteuserfield.invite_not_found', defaultMessage:  '{email} was not found'}, {email: _this.state.userEmail})
			);
		});
	}
