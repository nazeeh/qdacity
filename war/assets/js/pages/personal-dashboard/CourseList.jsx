...
export default class CourseList extends React.Component {
	...
	render() {
		...
		//Rebder List Items
		var filteredList = this.props.courses.filter(course => {
			return (
				course.name.toLowerCase().indexOf(this.state.search.toLowerCase()) !==
				-1
			);
		});
		...
		for (
			let i = 1;
			i <= Math.ceil(this.props.courses.length / this.state.itemsPerPage);
			i++
		) {
			pageNumbers.push(i);
		}
